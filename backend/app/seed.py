...
SEED_DATA_DIR = Path("/app/app/seed_data")
...
async def seed_hazards_via_api(client: httpx.AsyncClient) -> None:
    ...
    for tif_file in hazards_dir.glob("*.tif"):
        with open(tif_file, "rb") as f:
            files = {"wse_raster": (tif_file.name, f, "image/tiff")}
            data = {"name": f"{tif_file.stem.replace('_', ' ').title()}"}

            response = await client.post(f"{BASE_URL}/datasets/hazards", files=files, data=data)
...
async def seed_fragility_curves_via_api(client: httpx.AsyncClient) -> None:
    ...
    for json_file in fragility_dir.glob("*.json"):
        ...
        with open(json_file, "rb") as f:
            files = {"fragility_json": (json_file.name, f, "application/json")}
            data = {"name": curve_id}

            response = await client.post(f"{BASE_URL}/datasets/fragilities", files=files, data=data)
...
async def seed_mapping_sets_via_api(client: httpx.AsyncClient) -> None:
    ...
    for json_file in mapping_dir.glob("*.json"):
        with open(json_file, "rb") as f:
            files = {"mapping_json": (json_file.name, f, "application/json")}
            data = {"name": f"{json_file.stem.replace('_', ' ').title()}"}

            response = await client.post(f"{BASE_URL}/datasets/mappings", files=files, data=data)
...
async def seed_building_datasets_via_api(client: httpx.AsyncClient) -> None:
    ...
    for zip_file in buildings_dir.glob("*.zip"):
        with open(zip_file, "rb") as f:
            files = {"shapefile_zip": (zip_file.name, f, "application/zip")}
            data = {"name": f"{zip_file.stem.replace('_', ' ').title()}"}

            response = await client.post(f"{BASE_URL}/datasets/buildings", files=files, data=data)

            if response.status_code == 200:
                dataset = response.json()
                logger.info(
                    f"Seeded building dataset: {zip_file.name} with {dataset.get('feature_count', 0)} buildings"
                )
...
async def seed_interventions_via_api(client: httpx.AsyncClient) -> None:
    ...
    for intervention_data in interventions:
        response = await client.post(f"{BASE_URL}/interventions", json=intervention_data)

        if response.status_code == 200:
            logger.info(f"Seeded intervention: {intervention_data['name']}")
        else:
            logger.error(
                f"Failed to seed intervention {intervention_data['name']}: {response.text}"
            )
